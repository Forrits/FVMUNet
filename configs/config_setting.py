...
class setting_config:
    ...
    if datasets == 'isic18':
        ...
        data_path = './data/ACDC/'
        list_dir = './data/ACDC/train_slices'
        volume_path = './data/ACDC/val'
    else:
        raise Exception('datasets in not right!')

    #criterion = GT_BceDiceLoss(wb=1, wd=1)
    criterion=CeDiceLoss(num_classes=4)
    #criterion=BceDiceLoss()
    pretrained_path = 'pre_weight/vmamba_small_e238_ema.pth'
    num_classes = 4
    input_size_h = 224
    input_size_w = 224
    input_channels = 3
  ...
  # work_dir = 'results/' + network + '_' + datasets + '_' + datetime.now().strftime('%A_%d_%B_%Y_%Hh_%Mm_%Ss') + '/'
    # work_dir="Synapse/vmunet_result/"
    work_dir="ACDC/mambaunet_tiny_result/"
    print_interval = 20
    val_interval = 10
    save_interval = 50
    threshold = 0.5
